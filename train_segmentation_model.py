...
def train_encoders(config):
    ...
    optimizer = torch.optim.AdamW(
        list(segmentation_model.parameters()),
    ...
    for epoch in range(config['training']['epochs']):
        ...
        early_stopping.check_early_stop(val_loss, segmentation_model)

        print(f"\nEpoch {epoch + 1} completed with validation loss: {val_loss}\n")
    
        if early_stopping.stop_training:
            segmentation_model.load_state_dict(early_stopping.weights)
            print(f"Early stopping at epoch {epoch}")
            break

        torch.save(segmentation_model.state_dict(), f'./checkpoints/segmentation_vit_epoch_{epoch + 1}.pth')
